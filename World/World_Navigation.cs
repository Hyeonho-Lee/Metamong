    ...
    IEnumerator Create_House(float delay)
    {
        ...
        for (int i = 0; i < nullList.Count; i++) {
            if (0 <= nullList[i] && nullList[i] <= 14) {
                ...
                GameObject house = PhotonNetwork.Instantiate("photon_house_prefab/A/A_0", All_Position[nullList[i]].localPosition - new Vector3(0f, 10f, 0f), forcus);
        ...
        for (int i = 0; i < ac.h_position_index.Count; i++) {
            if (0 <= ac.h_position_index[i] && ac.h_position_index[i] <= 14 && ac.h_is_house[i]) {
                ...
                GameObject house = PhotonNetwork.Instantiate("photon_house_prefab/A/" + A_House_Prefab[ac.h_house_index[i]].name, All_Position[ac.h_position_index[i] - 1].localPosition - new Vector3(0f, 10f, 0f), forcus);
